...
CONFIG = {
    ...
    'BATCH_SIZE': 10,  # REDUCED to 10 for better memory management
    'STEP_SIZE': 60,
}
...
def create_tracker_satellite(scenario, config):
    """Create a satellite tracker with sensor"""
    try:
        tracker = AgSatellite(scenario.Children.New(AgESTKObjectType.eSatellite, config['name']))
        
        trackerGfx = tracker.Graphics.Attributes
        trackerGfx.Color = Color.FromRGB(0, 255, 0)
        trackerGfx.Line.Width = AgELineWidth.e1
        trackerGfx.Inherit = False
        ...
        sensor.Graphics.LineWidth = AgELineWidth.e1
...
def create_space_object(scenario, tle_data):
    """Create a space object from TLE data"""
    try:
        obj_name = f"Obj_{tle_data['norad_id']}"
        spaceObject = AgSatellite(scenario.Children.New(AgESTKObjectType.eSatellite, obj_name))
        
        objectGfx = spaceObject.Graphics.Attributes
        objectGfx.Color = Color.FromRGB(255, 0, 0)
        objectGfx.Line.Width = AgELineWidth.e1
        objectGfx.Inherit = False
...
def main():
    ...
    # Create ALL tracker satellites ONCE
    print(f"\n{'='*80}")
    print("CREATING ALL SATELLITE TRACKERS (ONE TIME)")
    print('='*80)
    
    sensors = []
    
    print(f"Creating {len(tracker_configs)} satellite trackers...")
    for i, config in enumerate(tracker_configs):
        if (i + 1) % 10 == 0:
            print(f"  Progress: {i+1}/{len(tracker_configs)} trackers created...")
    ...
    print(f"✓ Created {len(sensors)} satellite trackers with sensors")
    
    if len(sensors) == 0:
        print("ERROR: No trackers were created successfully. Exiting.")
        return
    
    # Process objects in batches
    print(f"\n{'='*80}")
    print("ANALYZING VISIBILITY IN BATCHES")
    print('='*80)
    ...
    for batch_idx in range(total_batches):
        ...
        print(f"  Computing visibility from {len(sensors)} trackers...")
        batch_results = []
        
        for i, sensor_info in enumerate(sensors):
            if (i + 1) % 20 == 0:
                print(f"    Progress: {i+1}/{len(sensors)} trackers processed...")
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"  Total trackers: {len(sensors)}\n")
    ...
    print('='*80)
    print(f"\nResults Summary:")
    print(f"  ✓ Trackers deployed: {len(sensors)}")
    ...
    print("\n✓ STK scenario remains open for visualization")
